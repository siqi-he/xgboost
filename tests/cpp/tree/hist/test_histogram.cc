...
// Test that sparse data with sorted bin indices produces correct histograms
// when the tiled kernel is used (threshold=1 forces tiling).
// Uses 10% sparsity so density (~90%) is above the kMinDensityForTiling=0.5 gate,
// ensuring the tiled sparse path is actually exercised.
TEST(CPUHistogram, SparseWithTiledKernel) {
  Context ctx;
  size_t constexpr kNRows = 512, kNCols = 64;
  int32_t constexpr kMaxBins = 32;
  // 0.1 sparsity = ~10% missing, ~90% density (above 0.5 tiling gate)
  auto p_fmat = RandomDataGenerator(kNRows, kNCols, 0.1).Seed(42).GenerateDMatrix();
  auto const &gmat =
      *(p_fmat->GetBatches<GHistIndexMatrix>(&ctx, BatchParam{kMaxBins, 0.5}).begin());
  ...
  for (size_t i = 0; i < kNRows; ++i) {
    gpair[i] = GradientPair(static_cast<float>(i) * 0.01f, 1.0f);
  }

  // Build histogram with tiled kernel (threshold=1 forces it on)
  HistogramBuilder hist_tiled;
  HistMakerTrainParam hist_param;
  hist_tiled.Reset(&ctx, total_bins, {kMaxBins, 0.5}, false, false, &hist_param);
  ...
  std::vector<bst_node_t> nodes{RegTree::kRoot};
  std::vector<bst_node_t> dummy_sub;
  hist_tiled.AddHistRows(tree.HostScView(), &nodes, &dummy_sub, false);

  common::BlockedSpace2d space{1, [&](std::size_t) { return row_set[0].Size(); }, 256};
  for (auto const &gidx : p_fmat->GetBatches<GHistIndexMatrix>(&ctx, {kMaxBins, 0.5})) {
  ...
  // Build reference histogram with original kernel (threshold=0 disables tiling)
  ...
  for (size_t i = 0; i < total_bins; ++i) {
    ASSERT_DOUBLE_EQ(hist_tiled.Histogram()[0][i].GetGrad(), hist_ref.Histogram()[0][i].GetGrad())
        << "Mismatch at bin " << i;
    ASSERT_DOUBLE_EQ(hist_tiled.Histogram()[0][i].GetHess(), hist_ref.Histogram()[0][i].GetHess())
        << "Mismatch at bin " << i;
  }
}
}  // namespace xgboost::tree
