...
void RowsWiseBuildHistKernel(Span<GradientPair const> gpair, Span<bst_idx_t const> row_indices,
  ...
  // Decide whether to use a thread-local buffer (tiled path).
  ...
  if constexpr (kAnyMissing) {
    ...
    if (static_cast<double>(hist_bytes) > usable_cache) {
      ...
      use_local_buffer = nnz > 2 * tile_overhead;
    }
  }

  std::vector<double> tl_buf;
  if (use_local_buffer) {
  ...
  // Row bounds. For dense (kAnyMissing=false), entries are stored at positions
  // 0..n_features-1 with global bin = gr_row[j] + offsets[j]. For sparse, entries
  // are stored consecutively with global bin = gr_row[j].
  ...
  for (size_t i = 0; i < size; ++i) {
    if constexpr (kAnyMissing) {
      ...
      row_starts[i] = get_rid(rid[i]) * n_features;
      row_sizes[i] = n_features;
    }
  }

  // When tiling, step by kColBlockSize so the local buffer stays cache-resident.
  // When not tiling, step by n_features so the outer loop runs once and writes
  // go directly to the main histogram (no clear/flush).
  const size_t block_step = use_local_buffer ? kColBlockSize : n_features;

  for (size_t cid_begin = 0; cid_begin < n_features; cid_begin += block_step) {
    const size_t cid_end = std::min(cid_begin + block_step, n_features);
    ...
    if (use_local_buffer) {
      target = tl_buf.data();
      std::fill_n(target, block_n_bins * 2, 0.0);
    } else {
      target = hist_data + two * bin_lo;
    }
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      size_t j = cursors[i];
      // Skip entries before the current block's bin range.
      while (j < row_size) {
        uint32_t gbin;
        if constexpr (kAnyMissing) {
          gbin = static_cast<uint32_t>(gr_row[j]);
        } else {
          gbin = static_cast<uint32_t>(gr_row[j]) + offsets[j];
        }
        if (gbin >= bin_lo) break;
        ++j;
      }
      // Scatter entries within the block.
      while (j < row_size) {
        uint32_t gbin;
        if constexpr (kAnyMissing) {
          gbin = static_cast<uint32_t>(gr_row[j]);
        ...
        if (gbin >= bin_hi) break;
        const uint32_t local_bin = two * (gbin - bin_lo);
        target[local_bin] += pgh_t[0];
        target[local_bin + 1] += pgh_t[1];
        ++j;
      }
      cursors[i] = j;
    }
...
void BuildHistDispatch(Span<GradientPair const> gpair, Span<bst_idx_t const> row_indices,
  ...
  if (BuildingManager::kReadByColumn) {
    ...
    RowsWiseBuildHistKernel<BuildingManager>(gpair, row_indices, gmat, hist);
  }
}
